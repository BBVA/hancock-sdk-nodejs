...
import { HancockTransactionService } from '..';
...
describe('ethereum client', async () => {
  ...
  let client: HancockTransactionService;
  const genericConfig = {
    host: 'genericHost',
    port: 1,
    base: 'genericBase',
  ...
  const signerMock: HancockSignerFn = jest.fn();
  const platformMock: SupportedPlatforms = SupportedPlatforms.ethereum;
  ...
  beforeEach(() => {
    ...
    client = new HancockTransactionService(config, platformMock, signerMock);
  ...
  it('should call sign correctly', async () => {
    ...
    expect(signerMock).toHaveBeenCalledWith(JSON.parse(rawtx), privatekey);
  ...
  it('should call sign without string correctly', async () => {
    ...
    expect(signerMock).toHaveBeenCalledWith(rawtx, privatekey);

  });

  it('should call signAndSend and sendToSignProvider with signProvider correctly', async () => {

    const sendTransactionToSignProviderSpy = jest.spyOn((HancockTransactionService.prototype as any), 'sendToSignProvider')
  ...
  it('should call signAndSend and sign and sendSigned with privateKey correctly', async () => {
    ...
    const signTransactionSpy = jest.spyOn((HancockTransactionService.prototype as any), 'sign')
      .mockImplementation(() => 'responseSignTransaction');
    const sendSignedTransactionSpy = jest.spyOn((HancockTransactionService.prototype as any), 'sendSigned')
      .mockImplementation(() => Promise.resolve('response'));
  ...
  it('should call signAndSend without options correctly', async () => {
    ...
    const sendTransactionSpy = jest.spyOn((HancockTransactionService.prototype as any), 'send')
      .mockImplementation(() => Promise.resolve('response'));
